...
/**
 * @author zh_zhou
 * created at 2019/06/25 11:41
 * Copyright [2019] [zh_zhou]
 */
public abstract class BaseCreateFileFix <T extends DartComponent> {
    ...
    public BaseCreateFileFix(@NotNull final DartClass dartClass) {
    ...
    public void invoke(@NotNull final Project project, final Editor editor, final PsiFile context) throws IncorrectOperationException {
        if (!FileModificationService.getInstance().prepareFileForWrite(context.getContainingFile())) return;
        evalAnchor(editor, context);
        processElements(project, editor,context, getElementsToProcess());
    }

    protected void processElements(@NotNull final Project project, @NotNull final Editor editor,@NotNull final  PsiFile file, @NotNull final Set<T> elementsToProcess) {
